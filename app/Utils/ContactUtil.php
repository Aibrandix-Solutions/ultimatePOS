    ...
    public function getContactInfo($business_id, $contact_id)
    {
        $contact = Contact::where('contacts.id', $contact_id)
            ...
            ->select(
                ...
                DB::raw("SUM(IF(t.type = 'purchase', (SELECT COALESCE(SUM(amount), 0) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id AND (transaction_payments.method != 'cheque' OR transaction_payments.cheque_status IS NULL OR transaction_payments.cheque_status = 'cleared')), 0)) as purchase_paid"),
                DB::raw("SUM(IF(t.type = 'purchase', (SELECT COALESCE(SUM(amount), 0) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id AND transaction_payments.method = 'cheque' AND transaction_payments.cheque_status = 'pending'), 0)) as purchase_pending_cheques"),
                DB::raw("SUM(IF(t.type = 'sell' AND t.status = 'final', (SELECT COALESCE(SUM(IF(is_return = 1,-1*amount,amount)), 0) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id AND (transaction_payments.method != 'cheque' OR transaction_payments.cheque_status IS NULL OR transaction_payments.cheque_status = 'cleared')), 0)) as invoice_received"),
                DB::raw("SUM(IF(t.type = 'sell' AND t.status = 'final', (SELECT COALESCE(SUM(amount), 0) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id AND transaction_payments.method = 'cheque' AND transaction_payments.cheque_status = 'pending'), 0)) as invoice_pending_cheques"),
