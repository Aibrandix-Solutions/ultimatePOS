    ...
    <div
        ...
        <div
            ...
            <div class="!tw-w-full md:!tw-w-none !tw-flex md:!tw-hidden !tw-flex-row !tw-items-center !tw-gap-3">
                @if (!Gate::check('disable_pay_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                    <button type="button"
                        ...
                        style="background: linear-gradient(135deg, #161160 0%, #2a2480 100%); border: 1px solid rgba(22,17,96,0.3);"
                ...
                @if (!Gate::check('disable_express_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                    <button type="button"
                        class="tw-font-bold tw-text-white tw-cursor-pointer tw-text-xs md:tw-text-sm tw-rounded-lg tw-p-3 tw-w-[5.5rem] tw-flex tw-flex-row tw-items-center tw-justify-center tw-gap-2 tw-shadow-lg tw-transition-all tw-duration-300 tw-hover:tw-scale-105 tw-hover:tw-shadow-xl @if (!$is_mobile)  @endif no-print @if ($pos_settings['disable_express_checkout'] != 0 || !array_key_exists('cash', $payment_types)) hide @endif pos-express-finalize @if ($is_mobile) col-xs-6 @endif"
                        style="background: linear-gradient(135deg, #28b77b 0%, #20a16b 100%); border: 1px solid rgba(40,183,123,0.2);"
                        data-pay_method="cash" title="@lang('tooltip.express_checkout')"> <i class="fas fa-money-bill-alt tw-mr-2"
            ...
            <div class="tw-flex tw-items-center tw-gap-4 tw-flex-row tw-overflow-x-auto">
                ...
                @if (!Gate::check('disable_express_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                    <button type="button"
                        class="tw-font-bold tw-text-white tw-cursor-pointer tw-text-xs md:tw-text-sm tw-rounded-lg tw-p-3 tw-w-[8.5rem] tw-hidden md:tw-flex lg:tw-flex lg:tw-flex-row lg:tw-items-center lg:tw-justify-center lg:tw-gap-1 tw-shadow-lg tw-transition-all tw-duration-300 tw-hover:tw-scale-105 tw-hover:tw-shadow-xl @if (!$is_mobile)  @endif no-print @if ($pos_settings['disable_express_checkout'] != 0 || !array_key_exists('cash', $payment_types)) hide @endif pos-express-finalize"
                        style="background: linear-gradient(135deg, #28b77b 0%, #20a16b 100%); border: 1px solid rgba(40,183,123,0.2);"
                        data-pay_method="cash" title="@lang('tooltip.express_checkout')"> <i class="fas fa-money-bill-alt tw-mr-2"
                            aria-hidden="true"></i> @lang('lang_v1.express_checkout_cash')</button>
                @endif
